...
@export var player_wood: int = 3 # стартовый запас дров для пополнения костров

signal active_campfire_changed(campfire)
...
func set_active_interactable(node: Node) -> void:
        ...
        _update_active_campfire(node)


func clear_active_interactable(node: Node) -> void:
        if node == active_interactable:
                active_interactable = null
        if node == active_campfire:
                active_campfire = null
                active_campfire_changed.emit(active_campfire)


func _update_active_campfire(node: Node) -> void:
        if node and node.is_in_group("campfire"):
                active_campfire = node
                active_campfire_changed.emit(active_campfire)
